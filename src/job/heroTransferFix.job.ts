...
export class HeroTransferFixJob extends BaseJob {
  process = async (fromBlock: number, toBlock: number) => {
    // if (fromBlock >= 13001920) {
    ...
    SystemConfigService.instance.heroTransferFixBlock = fromBlock;
  ...
  processHeroTransfer = async (transfer: EventData) => {
    const txDb = await TransferHero.findOne({
      where: { tx_hash: transfer.transactionHash },
    });
    if (txDb) {
      const h: any = await HeroV2.findOne({
        where: { token_id: transfer.returnValues.tokenId },
      });
      if (h && Number(h.block_number) >= transfer.blockNumber) return;
    }
    ...
    if (updateHero.length > 0) {
      for (let i = 0; i < updateHero.length; i++) {
        if (updateHero[i].owner === BURN_ADDRESS) {
          ...
          await HeroV2.update(updateHero[i], {
            where: {
              token_id: updateHero[i].token_id,
              block_number: {
                [Op.lt]: updateHero[i].block_number,
              },
            },
          });
        }
